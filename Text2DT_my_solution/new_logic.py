...
# json_data=json.load(open("data/Text2DT/Text2DT_tree_test.json","r"))
...
def revert_D1_D2(triple_divide):
    ...
    # import pdb;pdb.set_trace()
    return triple_divide
def get_all_data(triple_divide,text,tail_entitys_to_index):
    if "C" in triple_divide[0] and "C" in triple_divide[1]:
        import pdb;pdb.set_trace()
...
def get_append_d(text):
    # import pdb;pdb.set_trace()
    D=[]
    D_word=["不","推荐","以","来","暂","注意","使用","应用"]
    for and_word in D_word:
        
        if list(re.finditer(and_word,text)):
            hi=list(re.finditer(and_word,text))
            for item in hi[-1:]:
...
def revert_C_D(triple_divide, tail_entitys_to_index, text):
    reverse_words=["用于","是.*的首选","治疗.*效果","适合"]
...
def parse_tree_by_logit(json_data):
  ...
  for all_idx,ins_json_data in enumerate(json_data):
    ...
    for index ,triple in enumerate(ins_json_data['triple_list']):           
        tail_entitys_to_index.append([triple[0],triple[3],triple[0:3]])

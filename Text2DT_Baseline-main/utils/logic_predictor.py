...
def logic_pre_item(text,triples):
    index=[]
    and_words = ['且', '同时', '再', '然后', '联合', '继而', '并', '还需', '+', '与', '的',"所以"]
    or_words = ['或']
...
def logic_pre(text,tree):
    from copy import deepcopy
    for index,item in enumerate(deepcopy(tree)):
        triples=item["triples"]
        # import pdb;pdb.set_trace()
        # for triples in tree:
        tree[index]['logical_rel']=logic_pre_item(text,triples)
    return tree
